...
class InternetArchiveProvider : MainAPI() {
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return try {
            ...
            val result = mapper.readValue<SearchResult>(response)
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        return try {
            val response = app.get("$mainUrl/metadata/$identifier").text
            val result = mapper.readValue<MetadataResponse>(response)
            ...
            throw ErrorLoadingException("Internet Archive metadata could not be loaded")
        }
    }

    private data class SearchResult(
        val response: SearchDocs
    )

    private data class SearchDocs(
